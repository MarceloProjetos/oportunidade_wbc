# Fachada MCP — ServidorIntegracaoSAP (Fase 0)
...
## Tools (Fase 0 — leitura)

| Tool | Endpoint | Chave? |
|---|---|---|
| `verificar_saude(checks?, strict?)` | `GET /status` | não (aberto) |
| `listar_sincronizacoes_os(limit?)` | `GET /historico` | sim |
| `listar_sincronizacoes_oportunidades(limit?)` | `GET /oportunidades/historico` | sim |
| `info_oportunidades()` | `GET /oportunidades/info` | sim |
| `listar_pedidos_com_os(limit?)` | `GET /ordens-servico/disponiveis` | sim |
...
*"últimas sincronizações de OS?"*, *"quais pedidos têm OS disponível?"*.
